...
pub fn makeObject(env: napi.Env) !napi.Value {
    ...
    try object.setNamedProperty("null-key", try env.getNull());
    try object.setNamedProperty("true-key", try env.getBoolean(true));
    try object.setNamedProperty("false-key", try env.getBoolean(false));
